    ...
    <>
      <div className="flex flex-col items-center p-8 rounded-t-md">
        ...
        <div className="pb-6 flex flex-col space-y-4 items-center border-b w-full">
          <h1
            className="mx-auto w-[70%] montserrat-subtitle text-[#6c8492]  md:text-3xl 
        text-4xl  font-bold"
          > Unlock the
            <span className="montserrat-subtitle md:text-3xl 
        text-4xl font-extrabold text-[#6c8492] "> power of secrecy</span> with Cryptia Protocol</h1>

          {note === true &&
            <p
              className="montserrat-small text-[#6c8492] font-semibold font-mono w-[80%]">
              Guard the key, unleash the cr. Never reveal the 'secret key' , only share your secure 'Cr address' for confidential transactions. </p>}
        </div>
        ...
        <div className="my-6 flex gap-4 items-center p-2 px-3 rounded-md  bg-[#d1f5e5]">
          <p className="montserrat-small  font-semibold text-[#435864]">
            ...
            - #{cryptiaaddress}</p>
          <AiOutlineCopy className='font-bold text-2xl text-[#6c8492]' onClick={copy} />
        </div>
